...
END_A = NodalSupport(
    displacement_conditions={"X": FX, "Y": FX, "Z": FX}, rotation_conditions={"X": FX, "Y": FR, "Z": FR}
)
END_B = NodalSupport(
    displacement_conditions={"X": FR, "Y": FX, "Z": FX}, rotation_conditions={"X": FR, "Y": FR, "Z": FR}
)
...
def main():
    ...
    for typ, t in targets:
        ...
        results.append(
            {
                "quantity": f"{typ} mode ~{t} Hz",
                "target": t,
                "fers": round(freqs[best], 3),
