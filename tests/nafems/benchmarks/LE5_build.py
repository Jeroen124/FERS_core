...
def build(n_el=4):
    ...
    nodes[0].nodal_support = NodalSupport(
        ...
        warping_condition=FX,
    )  # restrained warping at the clamped end
    c.add_member_set(
        MemberSet(
...
def main():
    ...
    phi2 = (T_TORQUE * lam / (G * SEC["j"])) * (
        math.tanh(lam * L) * math.cosh(lam * X_A) - math.sinh(lam * X_A)
    )
    ...
    print(
        f"  vs NAFEMS -108 MPa: err {err_target:.2f}%   vs Vlasov {sig_analytical / 1e6:.2f} MPa: err {err_vlasov:.2f}%"  # noqa: E501
    )
        ...
        json.dump(
            {
                ...
                "results": [
                    {
                        "quantity": "axial warping stress sigma_xx at flange tip A (x=2.5 m), via 7-DOF warping beam",  # noqa: E501
                        "target": TARGET,
                        "fers": -round(sigma, 1),
                        "error_pct": round(err_target, 3),
                        "unit": "Pa",
                        "mesh_or_modes": "1 warping beam, 4 elements",
                    }
                ],
                "matched": err_target < 3.0,
                "method": "Vlasov restrained-warping torsion; sigma = bimoment * omega_n / C_w",
            },
            fh,
            indent=2,
        )
