...
def _sup(ux, uy):
    ...
    return NodalSupport(
        displacement_conditions={"X": ux, "Y": uy, "Z": FX}, rotation_conditions={"X": FX, "Y": FX, "Z": FX}
    )
...
def build(Nth, Ns):
    ...
    for k, (it, node) in enumerate(outer_nodes):
        ...
        def arclen(a, b):
            pa = outer((math.pi / 2) * a / Nth)
            pb = outer((math.pi / 2) * b / Nth)
            return math.hypot(pb[0] - pa[0], pb[1] - pa[1])

        trib = 0.0
        if it > 0:
            trib += 0.5 * arclen(it - 1, it)
        if it < Nth:
            trib += 0.5 * arclen(it, it + 1)
                ...
                "results": [
                    {
                        ...
                        "unit": "Pa",
                        "mesh_or_modes": f"{Nth}x{Ns} membrane",
                    }
                ],
                "convergence": conv,
                "matched": matched,
