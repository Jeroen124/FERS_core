...
from fers_core import (FERS, Node, Member, MemberSet, NodalSupport, Material, Section,
                       AnalysisOrder, Plate, LoadCase, SurfaceLoad, SurfaceLoadVertex,
                       SupportCondition, NodalMass)
...
def f1_free_free_beam():
    ...
    c = FERS(); c.settings.analysis_options.order = AnalysisOrder.LINEAR
    sec = Section(name="SQ", material=steel(), i_y=I, i_z=I, j=J, area=A)
    FX, FR = SupportCondition.fixed(), SupportCondition.free()
    plan = NodalSupport(displacement_conditions={"X": FR, "Y": FR, "Z": FX},
                        rotation_conditions={"X": FX, "Y": FX, "Z": FR})
    nodes = [Node(L * i / N, 0, 0) for i in range(N + 1)]
    for nd in nodes:
        nd.nodal_support = plan
    c.add_member_set(MemberSet(members=[Member(start_node=nodes[i], end_node=nodes[i + 1], section=sec) for i in range(N)]))
    try:
    ...
    return n_rigid == 3, f"free-free solved; {n_rigid} rigid-body modes (expect 3); freqs={[round(f,3) for f in freqs[:6]]}"
...
def f2_nodal_mass():
    ...
    return ok, f"tip-mass f1={freqs[0]:.4f} Hz (bare 3.34, target ~{f_target:.3f}) -> {'APPLIED' if freqs[0] < 0.5 else 'IGNORED (F2 not in wheel)'}"
...
def f4_plate_shear():
    ...
    c = FERS(); c.settings.analysis_options.order = AnalysisOrder.LINEAR
    mat = steel()
    fix = NodalSupport(); cols = []
    for ix in range(NX + 1):
        s = fix if ix == 0 else None
        cols.append((Node(X=LEN * ix / NX, Y=0.0, Z=0.0, nodal_support=s),
                     Node(X=LEN * ix / NX, Y=WID, Z=0.0, nodal_support=s)))
    for ix in range(NX):
        n00, n01 = cols[ix]; n10, n11 = cols[ix + 1]
        c.add_plate(Plate(nodes=[n00, n10, n11], material=mat, thickness=T, local_x_direction=(1, 0, 0), theory="Mindlin"),
                    Plate(nodes=[n00, n11, n01], material=mat, thickness=T, local_x_direction=(1, 0, 0), theory="Mindlin"))
    lc = LoadCase(name="P")
    SurfaceLoad(load_case=lc, magnitude=1000.0, direction=(0, 0, -1), polygon=[
        SurfaceLoadVertex(0, 0, 0), SurfaceLoadVertex(LEN, 0, 0), SurfaceLoadVertex(LEN, WID, 0), SurfaceLoadVertex(0, WID, 0)])
    c.add_load_case(lc)
...
if __name__ == "__main__":
    checks = [("F1 free-free modal", f1_free_free_beam),
