...
class Attribution(BaseModel):
    ...
    generated_by: str = Field(..., description='Always "FERS".')
    html: str = Field(..., description='Ready-to-inject HTML anchor, for web UIs.')
    text: str = Field(
...
class CalcStep(BaseModel):
    ...
    capacity: float | None = Field(
        None,
        description='The resistance `demand` was compared against, in display units and in\n`unit`. Present exactly when `demand` is.',
    )
    demand: float | None = Field(
        None,
        description='The action effect behind `value`, in display units, when this step\ncompares a single physical quantity against a single resistance — an\naxial force against `N_b,Rd`, a moment against `M_c,Rd`. Absent on\ndimensionless rows (the §6.2.1 and §6.3.3 interaction sums), where no\none demand and no one capacity exist to report.\n\n`value` stays the utilization ratio on every row, so a consumer that\nonly reads `value` is unaffected by these being present.',
    )
    formula: str
    label: str
    substituted: str
...
class Ec3SectionParams(BaseModel):
    ...
    a_eff: float | None = Field(
        None,
        description="Effective area for class 4 sections (EN 1993-1-5 §4.3), in the model's\narea unit. Required to run any EC3 check on a class 4 section: the gross\narea would overstate every axial resistance. A concentrically loaded\nstrut needs only this; `wel_y_eff` becomes required once there is a\nmoment to resist.",
    )
    ...
    section_class: conint(ge=0) | None = Field(
        None,
        description='Cross-section class 1–4 (EN 1993-1-1 §5.5). Class 4 needs `a_eff`, and\n`wel_y_eff` too once the member carries bending.',
    )
...
class EffectiveLengthValue(BaseModel):
    axial_force: float = Field(
        ...,
...
class MemberEffectiveLengths(BaseModel):
    about_y: EffectiveLengthValue | None = None
    about_z: EffectiveLengthValue | None = None
    member_id: conint(ge=0)


class MemberEndLabel(Enum):
    start = 'start'
    end = 'end'
...
class ModalAnalysisSettings(BaseModel):
    include_geometric_stiffness: bool | None = Field(
        None,
    ...
    stiffness_reference: EigenLoadRef | None = None
...
class MomentRotationCurve(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    end: CurveEndBehaviour | None = Field(
        None, description='Behaviour above the last tabulated point.'
    )
    ...
    start: CurveEndBehaviour | None = Field(
        None,
...
class NodeForces(BaseModel):
    bw: float | None = Field(None, description='Warping bimoment ([F·L²]).')
    fx: float = Field(..., description='Axial force (tension-positive).')
    fy: float = Field(..., description='Shear force about the local/global y axis.')
    fz: float = Field(..., description='Shear force about the local/global z axis.')
    mx: float = Field(..., description='Torsional moment about the x axis.')
    my: float = Field(..., description='Bending moment about the y axis.')
    mz: float = Field(..., description='Bending moment about the z axis.')
...
class SolverDiagnostics(BaseModel):
    ...
    converged: bool | None = Field(
        None, description='Whether the solve converged. Linear solves report `true`.'
    )
    ...
    residual_norm: float | None = Field(
        None,
...
class AnalysisOptions(BaseModel):
    ...
    nonlinear_method: NonlinearMethod | None = None
    order: AnalysisOrder
    ...
    tolerance: float = Field(
        ...,
        description='Convergence tolerance for the second-order Newton residual.\n\n**Floored at 1e-6.** A smaller value is accepted and then raised to that\nfloor, because below it the residual is dominated by rounding in the\nassembly rather than by the equilibrium error, so iterating further buys\nprecision that is not there. Asking for 1e-10 therefore gets you 1e-6, and\nthat is deliberate — the number is named `SECOND_ORDER_RESIDUAL_FLOOR` in\n`models::fers::fers`.\n\nIgnored entirely by a first-order (`Linear`) analysis, which factorises\nonce and does not iterate.',
    )


class BucklingAnalysisSettings(BaseModel):
    all_combinations: bool | None = Field(
        None,
    ...
    member_effective_lengths: bool | None = Field(
        None,
    ...
    participation_threshold: float | None = Field(
        None,
    ...
    reference: EigenLoadRef | None = None
    references: list[EigenLoadRef] | None = Field(
        None,
...
class BucklingModeClassification(BaseModel):
    harmonic: conint(ge=0) | None = Field(
        None,
    ...
    label: BucklingModeLabel = Field(
        ..., description='Suggested mode character (see label rules above).'
    )
    participating_node_fraction: float = Field(
        ...,
    ...
    top_node_energy_fraction: float = Field(
        ...,
    ...
    translation_fractions: list[float] = Field(
        ...,
...
class LinearizedHingeStiffness(BaseModel):
    dof: HingeDofLabel
    end: MemberEndLabel
    member_id: conint(ge=0)
...
class LinearizedSupportStiffness(BaseModel):
    dof: SupportDofLabel
    node_id: conint(ge=0)
    stiffness: float = Field(
        ...,
...
class MemberResult(BaseModel):
    ...
    member_displacement_peak: MemberDisplacementSample | None = None
...
class Results(BaseModel):
    ...
    solver_diagnostics: SolverDiagnostics | None = Field(
        None,
...
class BucklingMode(BaseModel):
    classification: BucklingModeClassification | None = None
    critical_load_factor: float = Field(
        ...,
    ...
    participation_by_member_set: dict[constr(pattern=r'^[0-9]+$'), float] | None = (
        Field(
            None,
...
class BucklingResults(BaseModel):
    linearized_hinge_stiffness: list[LinearizedHingeStiffness] | None = Field(
        None,
    ...
    linearized_support_stiffness: list[LinearizedSupportStiffness] | None = Field(
        None,
    ...
    member_effective_lengths: list[MemberEffectiveLengths] | None = Field(
        None,
...
class BucklingRun(BaseModel):
    ...
    results: BucklingResults | None = None
...
class MemberHinge(BaseModel):
    ...
    id: conint(ge=0)
    max_bimoment_warp: float | None = Field(
        None,
        description='See [`Self::max_tension_vx`] — a design capacity, not read by the solver.',
    )
    max_moment_mx: float | None = Field(
        None,
        description='See [`Self::max_tension_vx`] — a design capacity, not read by the solver.',
    )
    max_moment_my: float | None = Field(
        None,
        description='See [`Self::max_tension_vx`] — a design capacity, not read by the solver.',
    )
    max_moment_mz: float | None = Field(
        None,
        description='See [`Self::max_tension_vx`] — a design capacity, not read by the solver.',
    )
    max_tension_vx: float | None = Field(
        None,
    ...
    max_tension_vy: float | None = Field(
        None,
        description='See [`Self::max_tension_vx`] — a design capacity, not read by the solver.',
    )
    max_tension_vz: float | None = Field(
        None,
        description='See [`Self::max_tension_vx`] — a design capacity, not read by the solver.',
    )
...
class ResultsBundle(BaseModel):
    ...
    buckling_runs: list[BucklingRun] | None = Field(
        None,
